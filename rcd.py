...
class _ExportMixin:
	...
	def _generate_file_properties(self):
		"""
		"""

		return {
			'title': f'Hasil kalkulasi RC {" ".join(self.rc_element)} tanggal {self.t0.strftime("%d-%m-%Y")} s/d {self.t1.strftime("%d-%m-%Y")}',
	...
	def _generate_filename(self):
		...
		return f'RC_Output_{"_".join(self.rc_element)}_{self.t0.strftime("%Y%m%d")}-{self.t1.strftime("%Y%m%d")}'

	def _generate_summary(self, filepath:str, ):
		"""
		"""
		...
		return [
			...
			('', ''),
			('STATISTICS', ''),
			('Marked', statistic['marked']['total']),
			('Unused-marked', statistic['marked']['unused']),
			('Success-marked', statistic['marked']['success']),
			('Failed-marked', statistic['marked']['failed'])
		]

	def _worksheet_writer(self, workbook:xlsxwriter.Workbook, sheet_name:str, sheet_data:pd.DataFrame, *args):
		"""
		"""
	...
	def to_excel(self, **kwargs):
		...
		output_file_properties = self._generate_file_properties()
		...
		with xlsxwriter.Workbook(output_filepath) as wb:
			...
			for name, sheet in sheets_data.items():
				self._worksheet_writer(wb, name, sheet, rc_only_extension_col)

			# Write worksheet info
			ws_info = wb.add_worksheet('Info')
			rows = self._generate_summary(filepath=output_filepath)
...
class RCD(_ExportMixin, _RCBaseCalculation):

	def __init__(self, data:pd.DataFrame=None, calculate_bi:bool=False, check_repetition:bool=True, **kwargs):
		super().__init__(data, calculate_bi, check_repetition, **kwargs)


class SOEtoRCD(_ExportMixin, _SOEAnalyzer, _RCBaseCalculation):
